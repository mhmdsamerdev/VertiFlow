...
interface Condition { sensor_type: string; operator: string; value: number }
interface AlertForm {
  name: string; severity: string; conditions: Condition[]; channels: string[]
}
const emptyForm = (): AlertForm => ({
  name: '', severity: 'warning',
  conditions: [{ sensor_type: 'air_temp', operator: '>', value: 35 }],
  channels: [],
})
...
function ConditionRow({ cond, idx, onChange, onRemove }: {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <input type="number" step="0.1" value={cond.value}
...
function AlertFormBody({ form, setForm }: { form: AlertForm; setForm: (f: AlertForm) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <button onClick={() => setForm({...form, conditions: [...form.conditions, {sensor_type:'air_temp', operator:'>',value:30}]})}
...
export function AlertsSection() {
  ...
  async function handleAdd() {
    ...
    await withSaving(() => createAlertConfig({
      ...
      conditions: form.conditions, channels: form.channels,
    }))
  ...
  async function handleDelete(id: string) {
    ...
    await withSaving(() => deleteAlertConfig(id))
  }

  async function handleToggle(id: string) {
    const a = alertConfigs.find(x => x.id === id)
    if (!a) return
    await withSaving(() => updateAlertConfig(id, { enabled: !a.enabled }))
  }
    ...
    <div className="p-6 max-w-3xl">
      ...
      {showAdd && (
        <div className="card p-4 mb-6">
          ...
          <AlertFormBody form={form} setForm={setForm} />
